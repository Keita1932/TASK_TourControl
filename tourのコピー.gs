...
function tour2() {
  ...
  const output_sheet = ss.getSheetByName("tourのコピー");
  ...
  if (result.jobComplete && result.rows) {
    ...
    if (rows.length > 0) {
      // 出力の直前に前の値をクリア
      output_sheet.getRange(2, 1, output_sheet.getMaxRows() - 1, 15).clearContent();
...
function related_tour() {
  ...
  if (result.jobComplete && result.rows) {
    ...
    // 既存のsubmission_idを取得
    const submissionIdRange = output_sheet.getRange(2, 11, output_sheet.getLastRow() - 1);
    const submissionIdValues = submissionIdRange.getValues();
    const filteredSubmissionId = submissionIdValues.flat().filter(id => id !== '');


    // Filter rows based on matching submission_ids
    const filteredRows = rows.filter(row => filteredSubmissionId.includes(row[10])); // 10 is the index for submission_id in the rows array
   
    const updatedRows = groupAndAssignNumbers(filteredRows);
   
    if (updatedRows.length > 0) {
      output_sheet.getRange(2, 19, output_sheet.getLastRow() - 1,15).clearContent();
      output_sheet.getRange(2, 19, updatedRows.length, updatedRows[0].length).setValues(updatedRows);
    }
  } else {
    Logger.log("No data returned or the job is not complete.");
  }
